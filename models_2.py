...
- Hyperparameter tuning via RandomizedSearchCV — ALL models tuned, not just trees
  · Linear Regression → Ridge (alpha grid) + ElasticNet (alpha × l1_ratio)
  · Logistic Regression → C × penalty × solver grid
  · Decision Tree → depth × leaf × split grids
  · Random Forest → estimators × depth × features × leaf grids
  · XGBoost/LGBM/CatBoost → estimators × depth × lr × subsample × colsample
  · n_iter scales with grid size (min 20, up to 60 for large spaces)
  · All best params saved to reports/tuning_summary_block{1,2}.csv
  · Score-distribution plots saved (tune_trials_*.png)
- LightGBM/CatBoost fallback chain (no more “use weaker models”)
- SMOTE oversampling for class imbalance (Block 2)
- Engineered features: day_of_week, session_period, interaction terms
- SHAP feature importance instead of raw impurity importance
- Calibration plots for classifiers
- Learning curves to diagnose over/underfitting
- Model persistence (joblib) for best models
- Logging module instead of bare print statements
- Statistical significance tests (Wilcoxon) between top models
- Business-cost–aware threshold optimisation (FP/FN cost ratio)
...
RocCurveDisplay, CalibrationDisplay, brier_score_loss,
)
...
try:
from lightgbm import LGBMClassifier as XGBClassifier, LGBMRegressor as XGBRegressor  # noqa: F401
...
```
        class XGBClassifier(CatBoostClassifier):      # thin wrapper  # noqa: F811
            def __init__(self, **kw):
                kw.setdefault("verbose", 0)
                super().__init__(**kw)

        class XGBRegressor(CatBoostRegressor):         # noqa: F811
            def __init__(self, **kw):
                kw.setdefault("verbose", 0)
                super().__init__(**kw)
...
DEFAULT_COST_FP = 1.0   # cost of predicting return when user won’t (wasted spend)
...
paths = []
if ru_tsvs:  paths.append(ru_tsvs[-1])
if eng_tsvs: paths.append(eng_tsvs[-1])
if not paths:
...
if "hour_of_day" in sessions.columns:
    hour = pd.to_numeric(sessions["hour_of_day"], errors="coerce").fillna(12)
...
if "date" in df.columns:
    ...
    sessions["day_of_week"] = sessions["_date"].dt.day_name().fillna("Unknown")
    sessions["is_weekend"]  = sessions["_date"].dt.dayofweek.isin([5, 6]).astype(int)
...
available_cat = [c for c in SESSION_CAT if c in d.columns or c == “is_weekend”]
...
log.info(”    Tuning %-22s  grid_size=%-6d  n_iter=%d”, name, grid_size, n_iter)
...
log.info("    %-22s  best_score=%.4f  params=%s", name, score, search.best_params_)
...
if tune:
    _save_tuning_summary(summary, cv_results, "block1", "neg_RMSE (higher=better)")
...
_significance_table(fold_scores, metric=“R²”, output_name=“regression_significance.csv”)
return df
...
for ax, (name, imp) in zip(axes, importances.items()):
    ...
    ax.set_xlabel("Importance"); ax.set_title(name, fontsize=11)
    ax.invert_yaxis()
...
if len(linear_names) == 1: axes = [axes]
...
for ax, name in zip(axes, linear_names):
    ...
    ax.set_xlabel("|coefficient|"); ax.set_title(name, fontsize=11)
    ax.invert_yaxis()
...
for ax, name in zip(axes, target_names):
    ...
    try:
        ...
        ax.barh([feature_cols[i] for i in idx], mean_abs[idx],
                color="steelblue", alpha=0.85)
        ax.set_xlabel("|SHAP|"); ax.set_title(name, fontsize=11)
        ax.invert_yaxis()
...
len(sessions), sessions[“levels_completed”].mean(), sessions[“levels_completed”].max()
)
...
“hour_of_day”,
# ── new engineered ──────────────────────────────────────────────────────
“engagement_score”,    # completion_rate × session_duration (interaction term)
...
if ret == 0 or ret == total:
    log.warning("Only one class present — need both retained and non-retained users.")
    return
if min(ret, total - ret) < 5:
    log.warning("Very few samples in minority class — results will be unreliable.")
...
log.info("=" * 65)
print(results[["Accuracy", "Precision", "Recall", "F1", "ROC-AUC", "Brier"]].round(3).to_string())
log.info("=" * 65)
...
try:
    ...
    opt_t = optimise_threshold(best_model, X, y, cost_fp=cost_fp, cost_fn=cost_fn)
